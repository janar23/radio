...
public abstract class RadioPlayer {
    protected RadioPlayerCallback mCallback;
    ...
    public void setCallback(RadioPlayerCallback callback) {
        mCallback = callback;
    }

    public static RadioPlayer create(Context context, RouteInfo route) {
        RadioPlayer player;
        if (route != null && route.supportsControlCategory(
                MediaControlIntent.CATEGORY_REMOTE_PLAYBACK)) {
            player = new RemoteRadioPlayer(context);
        } else if (route != null) {
            player = new LocalRadioPlayer.SurfaceViewPlayer(context);
        } else {
            player = new LocalRadioPlayer.OverlayPlayer(context);
        }
